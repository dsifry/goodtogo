...
```bash
npx tsx scripts/beads-prime.ts --work-type review --keywords "security authentication authorization"
```
...
| Session        | Server-side session via NextAuth            |
...
// GOOD: userId from server session only
