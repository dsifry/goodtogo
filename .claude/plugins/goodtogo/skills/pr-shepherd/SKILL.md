...
Run `gtg` every 60 seconds to get deterministic PR status:

### Using gtg for PR Status

```bash
# Run gtg and capture JSON output
# Note: Default mode returns exit code 0 for all analyzable states
# Parse the JSON 'status' field to determine actual state
...
**Default (AI-friendly)**: Returns 0 for all analyzable states, 4 for errors only.
Parse the JSON `status` field to determine actual PR state.

**Semantic mode (`-q` or `--semantic-codes`)**: Returns different exit codes per status:

| Exit Code | Status | Action |
|-----------|--------|--------|
| 0 | `READY` | → DONE (all clear!) |
| 1 | `ACTION_REQUIRED` | → HANDLING_REVIEWS (actionable comments) |
| 2 | `UNRESOLVED_THREADS` | → HANDLING_REVIEWS (threads need resolution) |
| 3 | `CI_FAILING` | → FIXING (CI failures) |
| 4 | `ERROR` | → WAITING_FOR_USER (API error) |

### Evaluate State Transitions

**Recommended approach - parse JSON status:**
...
```bash
# Use -q for quiet mode with semantic exit codes
...
**⚠️ BLOCKING: You MUST run `gtg` and verify READY status before declaring ANY PR ready:**
...
**Default mode** (AI-friendly): Returns 0 for all analyzable states, 4 for errors only.
Parse the JSON `status` field to determine actual PR state.
...
| 2 | UNRESOLVED_THREADS | ❌ Unresolved review threads |
| 3 | CI_FAILING | ❌ CI checks failing |
| 4 | ERROR | ❌ Error fetching data |

**If status is NOT READY, you are NOT done.** Address each issue:
