...
The Security Auditor Agent performs thorough security review of code changes before PR creation. It identifies vulnerabilities based on OWASP Top 10 and Warmstart-specific security requirements. Any CRITICAL finding blocks the PR.
...
3. **Warmstart-Specific**: Verify Gmail, Stripe, PostHog security
...
```bash
npx tsx scripts/beads-prime.ts --work-type review --keywords "security" "authentication" "validation"
```
...
```typescript
// Check for:
// 1. Missing getServerSession() in API routes
// 2. Missing userId in database queries
...
// Pattern to verify:
prisma.model.findMany({ where: { userId: session.user.id } });
```
...
### Step 5: Warmstart-Specific Checks
...
**File**: `src/app/api/contacts/[id]/route.ts:23`
...
## Common Vulnerabilities in Warmstart
...
- `src/app/api/<route>` - Auth, input validation
