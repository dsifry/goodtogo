...
- [ ] **`scripts/service-registry.json`** updated (if new/modified service)
- [ ] **Mock factories** added/updated in `src/lib/services/mock-factories.ts` (if new service)
...
2. **Service Registry** - Check service metadata and categories:

   ```bash
   cat scripts/service-registry.json | jq '.services[] | select(.name | test("Search|Cache"; "i"))'
   ```

3. **Mock Factories** - Review existing mocks to reuse patterns:

   ```bash
   cat src/lib/services/mock-factories.ts
   ```

4. **Similar Services** - Find services with similar patterns:
   ```bash
