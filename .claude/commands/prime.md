...
```bash
npx tsx scripts/beads-prime.ts --keywords "authentication" "jwt" "security"
```
...
```bash
npx tsx scripts/beads-prime.ts --work-type planning
npx tsx scripts/beads-prime.ts --work-type implementation
npx tsx scripts/beads-prime.ts --work-type review
npx tsx scripts/beads-prime.ts --work-type debugging
```
...
- "ALWAYS use centralized AI config from src/lib/ai-config.ts"
...
- "Use mock factories from src/lib/services/mock-factories.ts"
...
```bash
npx tsx scripts/beads-prime.ts --work-type planning --keywords "<task-keywords>"
```
...
```bash
npx tsx scripts/beads-prime.ts --files "<files-to-touch>" --work-type implementation
```
...
```bash
npx tsx scripts/beads-prime.ts --work-type review --files "<files-changed>"
```
...
- **[pattern]** Use mock factories from src/lib/services/mock-factories.ts...
```
